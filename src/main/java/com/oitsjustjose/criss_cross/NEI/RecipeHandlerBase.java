			...
			for (FluidTankElement tank : crecipe.getFluidTanks())
			{
				if (tank.position.contains(relMouse))
				{
					if ((tank.fluid != null) && tank.fluid.amount > 0)
					{
						if (usage)
						{
							if (!GuiUsageRecipe.openRecipeGui("liquid", new Object[]
							{ tank.fluid }))
							{
								return false;
							}
						...
						{
							if (!GuiCraftingRecipe.openRecipeGui("liquid", new Object[]
							{ tank.fluid }))
							{
								return false;
							}
		...
		public void draw()
		{
			...
			for (int i = 0; i < this.position.width; i += 16)
			{
				for (int j = 0; j < amount; j += 16)
				{
					...
					tessellator.addVertexWithUV(drawX, drawY + drawHeight, 0, minU,
							minV + (maxV - minV) * drawHeight / 16F);
					tessellator.addVertexWithUV(drawX + drawWidth, drawY + drawHeight, 0,
							minU + (maxU - minU) * drawWidth / 16F, minV + (maxV - minV) * drawHeight / 16F);
					tessellator.addVertexWithUV(drawX + drawWidth, drawY, 0, minU + (maxU - minU) * drawWidth / 16F,
							minV);
